...
namespace SmartDose.DynamicClasses
{
    public static class DynamicClassFactory
    {
        ...
        private static string DynamicAssemblyName = "SmartDose.DynamicClasses, Version=1.0.0.0";
        private static string DynamicModuleName = "SmartDose.DynamicClasses";
            ...
            if (!GeneratedTypes.TryGetValue(key, out type))
            {
                lock (GeneratedTypes)
                {
                    if (!GeneratedTypes.TryGetValue(key, out type))
                    {
                        ...
                        TypeBuilder tb = ModuleBuilder.DefineType(name, TypeAttributes.Public | TypeAttributes.Sealed | TypeAttributes.BeforeFieldInit | TypeAttributes.Serializable, typeof(object));
